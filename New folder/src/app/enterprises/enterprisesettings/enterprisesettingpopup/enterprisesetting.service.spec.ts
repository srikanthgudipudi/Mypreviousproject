...
describe('EnterpriseSettingService', () => {
    let subject: EnterpriseSettingService;
    ...
    /*---- Jasmine runs beforeEach() before each of the tests ----*/
    beforeEach(inject([EnterpriseSettingService, MockBackend], (allfleettypelistingService, mockBackend) => {
    ...
    /*---- Test to verify the updated enterprise settings  ----*/
    it('should be called with proper arguments to get the updated enterprise settings', () => {
        backend.connections.subscribe((connection: MockConnection) => {
            expect(connection.request.url)
        ...
        subject.updateEnterpriseSettings('', '', '').subscribe((data) => {
            expect(data).toEqual({ success: true });
        });
    });
    /*---- Test to verify and get the fleet types list ----*/
    it('should be called with proper arguments to get the fleet types list', () => {
        backend.connections.subscribe((connection: MockConnection) => {
            expect(connection.request.url)
                .toEqual(config.API_END_POINT + '/api/v1/lookupsbyenterprise/FLEET_TYPES/0');
            expect(connection.request.method).toEqual(RequestMethod.Get);
            expect(connection.request.headers.get('Content-Type')).toEqual(null);
            const options = new ResponseOptions({
                body: JSON.stringify({ success: true })
