 ...
 * Original code from mapbbcode http://share.mapbbcode.org/
 * Modified by Sreenadh@Srisys
 */
L.FunctionButtons = L.Control.extend({
	includes: L.Mixin.Events,
	//Sreenadh: added selectedButtonIndex
    ...
    setSelectedButton: function(i){
        if (i<this._buttons.length)
            this.selectedButtonIndex=i;
        else
            this.selectedButtonIndex=this._buttons.length;
    },
	onAdd: function( map ) {
		this._map = map;
		this._links = [];
    	...
    	for( var i = 0; i < this._buttons.length; i++ ) {
			...
			//Sreenadh: Added selectedButton
            if (this.selectedButtonIndex!=null && this.selectedButtonIndex===i){
                link.style.backgroundColor = button.highColor;              
            } else
			if( button.bgColor )
	...
	clicked: function(e) {
		...
		if( '_buttonIndex' in link ) {
            ...
            if (this.selectedButtonIndex!=null)
  			   this._buttons[this.selectedButtonIndex].link.style.backgroundColor=this._buttons[this.selectedButtonIndex].bgColor;
            button.link.style.backgroundColor=button.highColor;
			this.selectedButtonIndex=link._buttonIndex;
			//end Sreenadh
			if( button ) {
